...
export const apiService = {
    ...
    async createInvoice(clientInfo: ClientInfo = { name: '', phone: '' }, globalTax = 0, globalDiscount = 0) {
        const response = await axios.post(`${config.backendUrl}/api/invoices`, {
            clientInfo,
            globalTax,
            globalDiscount
        });
    ...
    async getInvoice(id: string) {
        const response = await axios.get(`${config.backendUrl}/api/invoices/${id}`);
    ...
    async addItem(invoiceId: string, item: { description: string; quantity: number; unitPrice: number; tax: number; discount: number }) {
        const response = await axios.post(`${config.backendUrl}/api/invoices/${invoiceId}/items`, item);
        return response.data;
    },

    // Delete an item
    async deleteItem(itemId: string) {
        const response = await axios.delete(`${config.backendUrl}/api/invoices/items/${itemId}`);
        return response.data;
    },
    ...
    async updateItem(itemId: string, item: Partial<{ description: string; quantity: number; unitPrice: number; tax: number; discount: number }>) {
        const response = await axios.put(`${config.backendUrl}/api/invoices/items/${itemId}`, item);
        return response.data;
    },
    ...
    async updateInvoice(invoiceId: string, data: { clientInfo?: ClientInfo; globalTax?: number; globalDiscount?: number }) {
        const response = await axios.put(`${config.backendUrl}/api/invoices/${invoiceId}`, data);
        return response.data; // Returns FULL invoice object
    },

    async listUserDevis() {
        const response = await axios.get(`${config.backendUrl}/api/invoices`);
        return response.data;
    },

    async sendEmail(invoiceId: string, toEmail: string) {
        const response = await axios.post(`${config.backendUrl}/api/invoices/${invoiceId}/send-email`, { toEmail });
        return response.data;
    },

    async sendWhatsApp(invoiceId: string, toPhoneNumber: string) {
        const response = await axios.post(`${config.backendUrl}/api/invoices/${invoiceId}/send-whatsapp`, { toPhoneNumber });
        return response.data;
    }
};
